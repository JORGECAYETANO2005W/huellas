...
export const forgot_password = async (req, res) => {
    ...
    if (!secretWord) return res.status(400).json({ success: false, message: 'La palabra secreta es requerida' });
    ...
    try {
        ...
        // Verificar que la palabra secreta coincida
        if (user.secretWord !== secretWord) {
            return res.status(400).json({ 
                success: false, 
                message: 'La palabra secreta no coincide con nuestros registros' 
            });
        }
